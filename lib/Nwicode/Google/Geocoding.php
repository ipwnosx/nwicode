...
/**
 * Class Nwicode_Google_Geocoding
 */
class Nwicode_Google_Geocoding
{
    /**
    ...
    public static function geoReverse($latitude, $longitude, $apiKey = null)
    {
        ...
        $decode = Nwicode_Json::decode(file_get_contents($url));
    ...
    public static function testApiKey($apiKey = null)
    {
        $apiKey = trim($apiKey);
        if (empty($apiKey)) {
            throw new \Nwicode\Exception('#807-101 [GoogleMaps]: ' . __('Missing and/or empty API key.'));
        }

        $response = \Nwicode_Request::get("https://maps.googleapis.com/maps/api/geocode/json", [
            "sensor" => "false",
        ...
        if (empty($response)) {
            throw new \Nwicode\Exception('#807-102 [GoogleMaps]: ' . __('Something went wrong with the API.'));
        }

        $result = \Nwicode_Json::decode($response);

        if (!array_key_exists('status', $result)) {
            throw new \Nwicode\Exception('#807-103 [GoogleMaps]: ' . __('Something went wrong with the API.'));
        }

        if ($result['status'] !== 'OK') {
            throw new \Nwicode\Exception('#807-104 [GoogleMaps]: ' . $result['error_message']);
        }
