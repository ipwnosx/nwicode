...
use Nwicode\Cpanel\Api as CpanelApi;
...
class Cpanel
{
    ...
    public function updateCertificate($ssl_certificate)
    {
        ...
        if ($result) {
            $this->logger->info(__("[Nwicode_Cpanel] Updated cPanel SSL Certificate for %s", $webspace));
        } else {
            $this->logger->info(__("[Nwicode_Cpanel] Unable to update cPanel SSL Certificate for %s", $webspace));
            throw new Exception(__("[Nwicode_Cpanel] Unable to update cPanel SSL Certificate for %s", $webspace));
        }
