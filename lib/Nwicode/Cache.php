 ...
 *
 * Contains all common references for Nwicode_Cache_*
 */
...
class Cache
{
    ...
    /**
     * @param $pathFromNwicode
     */
    public static function __clearFolderSystem($pathFromNwicode)
    {
        $base = path($pathFromNwicode);
        exec("rm -rf {$base}/*");
    }
    ...
    public static function getDiskUsage($cache = false)
    {
        if (!$cache) {
            ...
            try {
                ...
                $logger->info(
                    'Nwicode_Cache::getDiskUsage() timeout ' .
                    $e->getMessage()
                );
            }
