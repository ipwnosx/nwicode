...
class Cron
{
    ...
    public function runTaskByCommand($command)
    {
        try {
            ...
            foreach ($tasks as $task) {
                if (!$task->getId()) {
                    throw new \Nwicode\Exception('The task doesn\'t exists.');
                }
                $this->execute($task);
            }
        } catch (Exception $e) {
    ...
    public function letsencrypt($task)
    {
        ...
        if (!$this->isLocked($task->getId())) {
            ...
            $letsEncrypt = new \Nwicode_LetsEncrypt($base, $root, false);
            $leIsInit = false;
                    ...
                    try {
                        ...
                        if (is_readable($cert->getCertificate()) &&
                            ...
                            if (isset($certContent["extensions"]) &&
                                ...
                                $dbHostname = \Nwicode_Json::decode($cert->getDomains());
                        ...
                        if ($renew) {
                            ...
                            # Save back domains!
                            $cert
                                ->setDomains(\Nwicode_Json::encode($retainDomains))
                                ->save();
    ...
    public function diskusage($task)
    {
        ...
        try {
            ...
            \Nwicode_Cache::getDiskUsage(true);
