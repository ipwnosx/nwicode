...
use \Nwicode\Cache\Design as Design;
...
class View extends \Zend_View
{
     ...
     */
    public function setBasePath($path, $classPrefix = 'Nwicode_View')
    {
        $path = rtrim($path, '/');
        $path = rtrim($path, '\\');
     ...
     * @return $this|\Zend_View_Abstract
     */
    public function addBasePath($path, $classPrefix = 'Nwicode_View')
    {
        $path = rtrim($path, '/');
        $path = rtrim($path, '\\');
        $path .= DIRECTORY_SEPARATOR;
    ...
    {
        /** Nwicode local inheritance test */
