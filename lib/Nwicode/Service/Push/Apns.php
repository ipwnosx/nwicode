...
class Nwicode_Service_Push_Apns extends ApnsPHP_Push {
    ...
    public function addMessage($push_message, $device) {
        ...
        $message = new Nwicode_Service_Push_Apns_Message($device->getDeviceToken());
        ...
        if($push_message->getLongitude() && $push_message->getLatitude()) {
            ...
            $user_info = $payload[Nwicode_Service_Push_Apns_Message::APPLE_RESERVED_NAMESPACE];

            # Build new message
            $geolocated_message = new Nwicode_Service_Push_Apns_Message();
