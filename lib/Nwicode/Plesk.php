...
class Nwicode_Plesk
{
    ...
    public function setCertificate($hostname)
    {
        try {
            ...
            if ($results === false) {
                ...
                $this->logger->info(sprintf("[Nwicode_Plesk] %s", $message));

                throw new Nwicode_Exception(__("[%s] %s", $request->error->getCode(), $message));
            }
            ...
            $plesk_browser = new Nwicode_Plesk_Crawler($this->config["host"], $this->config["username"], $this->config["password"]);
    ...
    public function removeCertificate($ssl_certificate)
    {
        ...
        $cert_name = sprintf("%s-%s", "nwicode_letsencrypt", $webspace);
        ...
        $this->logger->info(sprintf("[Nwicode_Plesk] First try to remove an existing one ..."));
        try {
            $request = new Plesk\SSL\DeleteCertificate($this->config, $params_delete);
            $info = $request->process();

            if (!$info) {
                $this->logger->info(sprintf("[Nwicode_Plesk] %s", $request->error->getMessage()));

                throw new Exception(__("[%s] %s", $request->error->getCode(), $request->error->getMessage()));
            }

            $this->logger->info(sprintf("[Nwicode_Plesk] %s", $request->xml_response));
            $this->logger->info(sprintf("[Nwicode_Plesk] %s", print_r($info, true)));
            $this->logger->info(sprintf("[Nwicode_Plesk] %s", "Certificate cleaned-up"));
        } catch (Exception $e) {
            $this->logger->info(sprintf("[Nwicode_Plesk] %s", $e->getMessage()));
            throw new Exception($e->getMessage());
        }
    ...
    public function updateCertificate($ssl_certificate)
    {
        ...
        $this->logger->info(sprintf("[Nwicode_Plesk] Installing the certificate ..."));
        try {
            ...
            $this->logger->info(sprintf("[Nwicode_Plesk] %s", $request->xml_response));
            $this->logger->info(sprintf("[Nwicode_Plesk] %s", print_r($info, true)));
            $this->logger->info(sprintf("[Nwicode_Plesk] %s", "Certificate installed"));
        } catch (Exception $e) {
            $this->logger->info(sprintf("[Nwicode_Plesk] Unable to install the certificate: %s", $e->getMessage()));
            $this->logger->info(sprintf("[Nwicode_Plesk] %s", print_r($request->error, true)));
            throw new Exception($e->getMessage());
        }
    ...
    public function selectCertificate($ssl_certificate)
    {
        ...
        $this->logger->info(sprintf("[Nwicode_Plesk] Trying to select the certificate and reloading ..."));
        if (version_compare(phpversion(), "5.3", ">")) {
            $this->logger->info(sprintf("[Nwicode_Plesk] %s", "Trying to select Certificate."));
            ...
            $this->logger->info(sprintf("[Nwicode_Plesk] %s", "Unable to set the current Certificate in Plesk, please check for PHP 5.6+."));
        ...
        $this->logger->info(sprintf("[Nwicode_Plesk] %s", "Done with success."));
