...
class Json extends Zend_Json
{
    ...
    public static function encode($data, $options = null, $withControl = false)
    {
        ...
        if ($json === false && (json_last_error() == JSON_ERROR_UTF8)) {
            ...
            log_warn(implode(
                "\n",
                ["Nwicode_Json::encode(), trying to force UTF-8", json_last_error_msg()]),
                "json_error_",
                false);
            ...
            try {
                ...
                $json = json_encode([
                    "error" => 1,
                    "message" => "Nwicode_Json::encode() UTF-8 failed",
                ]);

                log_warn(implode(
                    "\n",
                    ["Nwicode_Json::encode() UTF-8 failed", $e->getMessage()]),
                    "json_error_",
                    false);
            }
            ...
            log_warn(implode(
                "\n",
                ["Nwicode_Json::encode(), unhandeld error", json_last_error_msg()]),
                "json_error_",
                false);
        }
    ...
    public static function decode($json, $objectDecodeType = Zend_Json::TYPE_ARRAY)
    {
        ...
        if (is_null($result)) {
            log_warn(implode(
                "\n",
                ['Nwicode_Json::decode(), unable to decode json.', json_last_error_msg()]));
