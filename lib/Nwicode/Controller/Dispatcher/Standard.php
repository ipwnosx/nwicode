...
class Nwicode_Controller_Dispatcher_Standard extends Zend_Controller_Dispatcher_Standard
{
    ...
    public function loadClass($className)
    {
        ...
        if (self::isReadableInherit($loadFile)) {
            ...
            throw new Zend_Controller_Dispatcher_Exception('Nwicode:: Cannot load controller class "' . $className . '" from file "' . $loadFile . "'");
        }

        if (!class_exists($finalClass, false)) {
            require_once 'Zend/Controller/Dispatcher/Exception.php';
            throw new Zend_Controller_Dispatcher_Exception('Nwicode:: Invalid controller class ("' . $finalClass . '")');
        }
    ...
    public static function isReadableInherit($path, $return_path = false) {
        ...
        $type = strtolower(Nwicode_Version::TYPE);
        $editions = array_reverse(Nwicode_Cache::$editions[$type]);
