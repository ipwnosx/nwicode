...
/**
 * Class Nwicode_Migration_Db_Table
 *
 ...
 *
 */
class Nwicode_Migration_Db_Table extends Zend_Db_Table_Abstract
{
    ...
    /**
     * Nwicode_Migration_Db_Table constructor.
     *
    ...
    public function readSchema()
    {
        ...
        if (is_readable($this->schemaPath)) {
            ...
            throw new \Nwicode\Exception("Unable to read latest schema for '{$this->tableName}', errors: "
                . implode("\n", $this->queries) . ".");
        }
    }
...
\$schemas['{$this->tableName}'] = [";
    ...
    '{$name}' => [
        ...
        if ($save) {
            ...
            if (is_readable($schema_path)) {
                ...
                throw new \Nwicode\Exception("Unable to save schema from database for '{$this->tableName}'.");
            }
    ...
    public function updateTable()
    {
        ...
        $migration_log = sprintf($this->log_info, Nwicode_Version::VERSION);
    ...
    public function execSafe($query)
    {
        ...
        try {
            ...
            $migration_log = sprintf($this->log_error, Nwicode_Version::VERSION);
            $this->logger->info($message_error, $migration_log, false);
            throw new \Nwicode\Exception($message_error);
        }
    }
