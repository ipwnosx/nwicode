...
class Mcommerce_Mobile_Sales_PaymentController extends Mcommerce_Controller_Mobile_Default
{
        ...
        try {
            ...
            if ($cartTotal > 0) {
                ...
                if ($freeMethod->getId()) {
                    ...
                    throw new Nwicode_Exception('#635-01' . __('Unkown error!'));
                }
            }
        } catch(Exception $e) {
            $payload = [
        ...
        try {
            ...
            if ($params = Nwicode_Json::decode($request->getRawBody())) {
                ...
                if (empty($formValues['payment_method_id'])) {
                    throw new Nwicode_Exception(__('Please choose a payment method'));
                }
    ...
    public function validatepaymentAction() {
        try {
            ...
            if (empty($params)) {
                throw new Nwicode_Exception(__('Missing params!'));
            }
            ...
            try {
                ...
                if (empty($errors)) {
                    ...
                    array_key_exists('notes', $params) ?
                        $order->setNotes(\Nwicode\Xss::sanitize($params['notes'])) : $order->setNotes('');
                    $order->save();
