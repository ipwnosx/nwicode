...
class Promotion_Mobile_ListController extends Application_Controller_Mobile_Default {
    ...
    public function useAction() {
        ...
        try {
            ...
            if (!$customer_id) {
                throw new Nwicode_Exception(__('You must be logged in to use a discount'));
            }

            if ($data = Nwicode_Json::decode($this->getRequest()->getRawBody())) {

                if (empty($data['promotion_id'])) {
                    throw new Nwicode_Exception(__('An error occurred while saving. Please try again later.'));
                }
                ...
                throw new Nwicode_Exception(__('Missing data.'));
            }
        ...
        try {
            ...
            if ($data = Zend_Json::decode($this->getRequest()->getRawBody())) {
                ...
                if ($promotion->getUnlockCode() != $data['qrcode']) {
                    throw new Nwicode_Exception(__('This code is unrecognized'));
                }

                if ($promotion_customer->getIsUsed() != '' && $promotion_customer->getIsUsed() == 0) {
                    throw new Nwicode_Exception(__('You have already use this code'));
                }
                ...
                throw new Nwicode_Exception(__('An error occured.'));
            }
