 ...
 */
abstract class Core_View_Default_Abstract extends Nwicode\View
{
    ...
    {
        $path = Nwicode_Cache_Design::getBasePath("/images/" . $name);
        if (file_exists($path)) {
            return Nwicode_Cache_Design::getPath("/images/" . $name);
    ...
    protected function _renderZendMenu($xml)
    {
        ...
        if (!$this->getPluginLoader('helper')->getPaths('Nwicode_View_Helper_Navigation')) {
            $this->addHelperPath('Nwicode/View/Helper/Navigation', 'Nwicode_View_Helper_Navigation');
        }
