...
abstract class Core_Controller_Default_Abstract extends Zend_Controller_Action implements Core_Model_Exporter
{
    ...
    /**
     * @return Nwicode_Controller_Request_Http
     */
    ...
    public function init()
    {
        ...
        if (!empty($_FILES)) {
            \Nwicode\Security::filterFiles($_FILES, $session);
        }

        if (!empty($_GET)) {
            \Nwicode\Security::filterGet($_GET, $session);
        }

        if (!empty($_POST)) {
            \Nwicode\Security::filterPost($_POST, $session);
        }

        $bodyParams = $this->getRequest()->getBodyParams();
        if (!empty($bodyParams)) {
            \Nwicode\Security::filterBodyParams($bodyParams, $session);
        }
    }
        ...
        if (isset($this->cache_triggers) && is_array($this->cache_triggers)) {
            ...
            if (isset($this->cache_triggers[$action_name])) {
                ...
                if (isset($values["tags"]) && is_array($values["tags"])) {
                    ...
                    $payload_data = Nwicode_Json::decode($request->getRawBody());
    ...
    {
        return Nwicode_Version::TYPE == "SAE";
    }
    ...
    {
        return Nwicode_Version::TYPE == "MAE";
    }
    ...
    {
        return Nwicode_Version::TYPE == "PE";
    }
    ...
    public static function sGetColorizedImage($image_id, $color)
    {
        ...
        Nwicode_Media::disableTemporary();
    ...
    protected function _initDesign()
    {
        ...
        Nwicode_Cache_Design::$design_codes = $designCodes;
    ...
    protected function _initLocale()
    {
        ...
        /*manual country code*/
		if ($this->getRequest()->isApplication() && $this->getApplication()->getData('currency_code')!="") {
			try
			{
				$currency = new Zend_Currency($this->getApplication()->getData('currency_code'));
				Core_Model_Language::setCurrentCurrency($currency); 
			}
			catch(Exception $e)
			{
    ...
    public function _sendJson($payload, $options = JSON_PRETTY_PRINT)
    {
        ...
        $json = Nwicode_Json::encode($payload, $options);

        Nwicode_Debug::sendDataInHeaders();
    ...
    public function _download($file, $filename, $content_type = 'application/vnd.ms-excel')
    {
        ...
        if (file_exists($file)) {
            ...
            if (stripos($file, $sibTmpPath) !== 0 && stripos($file, $sibAppsPath) !== 0) {
                throw new \Nwicode\Exception("Forbidden path $file");
            }
    ...
    public function exportAction()
    {
        if ($this->getCurrentOptionValue()) {
            ...
            if (Nwicode_Exporter::isRegistered($option->getCode())) {
                $class = Nwicode_Exporter::getClass($option->getCode());
                $exporter = new $class();
