...
class Importer_Model_FacebookParser extends Core_Model_Default
{
    ...
    public function getToken()
    {
        ...
        if (!$facebookAPP_SECRET || !$facebookAPP_ID) {
            ...
            $content = Nwicode_Json::decode(Nwicode_Request::get($url));
    ...
    public function parsePage($page_id, $token, $fields = null)
    {
        ...
        $response = Nwicode_Request::get($url);
        $response = Nwicode_Json::decode($response);

        if (array_key_exists('error', $response)) {
            // Parsing the error
            $data = $response['error'];
            if (array_key_exists('message', $data)) {
                throw new Nwicode_Exception($data['message']);
            } else {
                throw new Nwicode_Exception(__('An unknown error occured with the Facebook API.<br />' . print_r($response, true)));
            }
        }
