    ...
    public function savelicenseAction()
    {
        try {
            if (Installer_Model_Installer::isInstalled() === false) {
                ...
                __set('nwicodecms_key', $request->getParam('key'));

                $payload = [
                    'success' => true
                ];
            } else {
                throw new Nwicode_Exception("An error occured while saving the license.");
            }
        } catch (Exception $e) {
            $payload = [
                'error' => true,
    ...
    public function endAction()
    {
        try {
            if (Installer_Model_Installer::setIsInstalled()) {
                ...
                __set('installation_version', Nwicode_Version::VERSION, 'Installation version');

                $payload = [
                    'success' => true
                ];
            } else {
                throw new Nwicode_Exception("An error occured while finalizing the installation.");
            }
        } catch (Exception $e) {
            $payload = [
                'error' => true,
