...
class Installer_Backoffice_ModuleController extends Backoffice_Controller_Default
{
        ...
        $payload = [
            ...
            'words' => [
                ...
                'majorMessage' =>
                    ...
                    "<br />" .
                    "<br />" .
                    "<a class=\"btn btn-primary\" href=\"https://updates.nwicode.com/release-notes/major/#VERSION#.html\">Version #VERSION# technical notes.</a>" .
                    "<br />" .
                    "<br />" .
    ...
    public function downloadupdateAction()
    {
        try {
            ...
            if ($fatalErrors) {
                throw new Nwicode_Exception(implode(', ', $_errors));
            }
            ...
            if (!empty($data['url']) && !empty($data['filename'])) {
                ...
                if (Nwicode_Version::TYPE === 'SAE') {
                    ...
                    $licenseKey = System_Model_Config::getValueFor('nwicodecms_key');
                    if (!$licenseKey) {
                        throw new Nwicode_Exception(__('There is no CMS license key set.'));
                    }
                ...
                if ($response->getStatus() == 200) {
                    ...
                    if (empty($content)) {
                        throw new Nwicode_Exception(__('#100: Unable to fetch the update. Please, try again later.'));
                    }

                    file_put_contents($tmp_path, $content);
                } else {
                    throw new Nwicode_Exception(__('#101: Unable to fetch the update. Please, try again later.'));
                }

                if (!file_exists($tmp_path)) {
                    throw new Nwicode_Exception(__('#102: Unable to fetch the update. Please, try again later.'));
                }
    ...
    public function uploadAction() {
        try {
            ...
            if (empty($_FILES) || empty($_FILES['file']['name'])) {
                throw new \Nwicode\Exception(__("No file has been sent"));
            }
    ...
    public function checkpermissionsAction() {
        if ($file = $this->getRequest()->getParam("file")) {
            try {
                ...
                if (!file_exists($file)) {
                    throw new \Nwicode\Exception(__("The file %s does not exist", $filename));
                }
                ...
                if (!$is_ok) {
                    ...
                    if ($ftp->checkConnection() && $ftp->isNwicodeDirectory()) {
                        $is_ok = true;
                    }
                }
                ...
                if ($is_ok) {
                    ...
                    $message = implode("\n", $messages);
                    throw new \Nwicode\Exception(__($message));
                }
        ...
        if($data = Zend_Json::decode($this->getRequest()->getRawBody())) {
            ...
            try {
                ...
                if(!$ftp->checkConnection()) {
                    $error_code = 1;
                    throw new \Nwicode\Exception(__("Unable to connect to your FTP. Please check the connection information."));
                } else if(!$ftp->isNwicodeDirectory()) {
                    $error_code = 2;
                    throw new \Nwicode\Exception(__("Unable to detect your site. Please make sure the entered path is correct."));
                }
    ...
    protected function _fetchUpdates() {
        ...
        if(Nwicode_Version::TYPE === "SAE") {
            ...
            $license_key = System_Model_Config::getValueFor("nwicodecms_key");
            if(!$license_key) {
                throw new Nwicode_Exception(__("There is no CMS license key set."));
            }
