...
class Installer_Model_Installer_Module_Parser extends Core_Model_Default
{
    ...
    public function extract()
    {
        ...
        if (!is_writable($tmp_dir)) {
            throw new \Nwicode\Exception(__("#19-001 The folder %s is not writable. Please fix this issue and try again.", $tmp_dir));
        } else {
    ...
    private function _checkDependenciesModule($package, $package_type = "module")
    {
        ...
        $updatepathurl = $package->getUpdatepathurl();
            ...
            switch ($type) {
                case "system":
                    if (!empty($values["type"])) {
                        ...
                        if ($_type > constant(Nwicode_Version::TYPE) || version_compare($_version, Nwicode_Version::VERSION, ">")) {
        ...
        if ($_module->isInstalled() && version_compare($package->getVersion(), $_module->getVersion(), "<=")) {
            ...
            $_module
                ...
                ->setUpdatepathurl($updatepathurl)
                ->save();
        }
    }
            ...
            foreach ($dependencies as $type => $dependency) {
                ...
                switch ($type) {
                    ...
                    case "system":
                        ...
                        if (strtolower($dependency["type"]) != strtolower(Nwicode_Version::TYPE)) {
                            throw new Exception(__("#19-003: This update is designed for the %s, you can't install it in your %s.", $package->getName(), Nwicode_Version::NAME));
                        }
                        ...
                        # If the current version of Nwicode equals the package's version
                        if (version_compare(Nwicode_Version::VERSION, $package->getVersion(), "=")) {
                            throw new Exception(__("#19-004: You already have installed this update."));
                        } elseif (version_compare(Nwicode_Version::VERSION, $dependency["version"], "<")) {
                    ...
                    case "module":
                        ...
                        $compare = version_compare(Nwicode_Version::VERSION, $dependency["version"]);
                        if ($compare == -1) {
    ...
    public function getPackageDetails()
    {
        if (!$this->_package_details) {
            ...
            if (!file_exists($package_file)) {
                throw new Exception(__("Your system is up to date."));
            }

            try {
                $content = Zend_Json::decode(file_get_contents($package_file));
            } catch (Zend_Json_Exception $e) {
                Zend_Registry::get("logger")->sendException(print_r($e, true), "nwicode_update_", false);
    ...
    private function __getFtp()
    {
        ...
        if (!$this->__ftp) {
            ...
            if ($host) {
                ...
                $this->__ftp = new Nwicode_Ftp($host, $user, $password, $port, $path);
            }
        }
