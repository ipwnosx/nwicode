...
class Installer_Model_Installer_Module extends Core_Model_Default
{
    ...
    public function install()
    {
        if ($this->canUpdate()) {
            ...
            foreach ($this->_schemaFiles as $table_name => $filename) {
                $migration_table = new Nwicode_Migration_Db_Table($table_name);
        ...
        if (is_readable($template_install_path)) {
            ...
            throw new \Nwicode\Exception('#437-001: ' .
    ...
    protected function fetchModule($module_name)
    {
        ...
        $editions = Nwicode_Cache_Design::$editions[strtolower(Nwicode_Version::TYPE)];
    ...
    public function getFeatures($refresh = false)
    {
        ...
        if ($this->_features === null || $refresh) {
            ...
            foreach ($featuresGlob as $feature) {
                $featureJson = Nwicode_Json::decode(file_get_contents($feature), true);
