        ...
        if($datas = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                if (empty($datas['value_id'])) {
                    throw new \Nwicode\Exception(__('An error occurred while saving. Please try again later.'));
                }
                ...
                if ($facebook->getId() AND $facebook->getValueId() != $option_value->getId()) {
                    throw new \Nwicode\Exception("Une erreur est survenue lors de la sauvegarde de votre galerie vidéos. Merci de réessayer ultérieurement.");
                }

                // Test connection
                $accessToken = Core_Model_Lib_Facebook::getAppToken();
                $testResponse = Nwicode_Request::get('https://graph.facebook.com/v2.7/' .
                    $datas['fb_user'] .
                    '?fields=id,about,name,genre,cover,fan_count,likes,talking_about_count&access_token=' .
                    $accessToken);
                $response = Nwicode_Json::decode($testResponse);

                if (array_key_exists('error', $response)) {
                    $data = $response['error'];
                    if (array_key_exists('message', $data)) {
                        throw new \Nwicode\Exception($data['message']);
                    } else {
                        throw new \Nwicode\Exception(__('An unknown error occured with the Facebook API.<br />' . print_r($response, true)));
                    }
                }
