 ...
 */
class Application_Form_Export extends Nwicode_Form_Abstract {
    ...
    public function addOptions($application) {
        ...
        foreach($application->getOptions() as $opt) {
            $option_id = $opt->getId();
            $feature = $opt->getCode();
            if(Nwicode_Exporter::isRegistered($feature)) {
                $label = __($opt->getName())." (".$opt->getTabbarName().")";
                if(Nwicode_Exporter::hasOptions($feature)) {
                    $options = Nwicode_Exporter::getOptions($feature);
