...
class Application_Controller_Mobile_Default extends Core_Controller_Default
{
    ...
    public function init()
    {
        ...
        Nwicode::setApplication($this->getApplication());
        ...
        if (!$this->getApplication()->valueIdBelongsTo($id) && $id) {
            ...
            Hook::trigger("mobile.controller.init", [
    ...
    public function _sendJson($data, $send = false)
    {
        ...
        if (!Nwicode_Debug::isDevelopment() && isset($data["exceptionMessage"])) {
            unset($data["exceptionMessage"]);
        }

        $json = Nwicode_Json::encode($data);
