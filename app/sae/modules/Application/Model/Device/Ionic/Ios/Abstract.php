...
abstract class Application_Model_Device_Ionic_Ios_Abstract extends Application_Model_Device_Abstract
{
    ...
    {
        if (!defined('CRON')) {
            $request = new Nwicode_Controller_Request_Http($this->currentApplication->getUrl());
            $request->setPathInfo();
    ...
    protected function ionicResources($application)
    {
        ...
        try {
            // Convert to jpeg
            $jpegStartup = Nwicode_Image::open($universal);
            ...
            Nwicode_Media::optimize($_tmpStartup);
    ...
    protected function buildPList()
    {
        ...
        $iPhone = [];
        $iPad = [];
        $orientations = Nwicode_Json::decode($device->getOrientations());
