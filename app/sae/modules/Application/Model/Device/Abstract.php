...
abstract class Application_Model_Device_Abstract extends Core_Model_Default
{
    ...
    public function setVersion ($version)
    {
        if (preg_match('/^(\d+\.)?(\d+\.)?(\*|\d+)$/', $version) !== 1) {
            throw new \Nwicode\Exception(__('The version number format is invalid, please use x.y.z where x, y & z are only digits.'));
        }
    ...
    protected function __replace($replacements, $file, $regex = false)
    {
        ...
        if (!$contents) {
            throw new \Nwicode\Exception(__('An error occurred while editing file (%s).', $file));
        }
