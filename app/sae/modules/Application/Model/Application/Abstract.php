...
abstract class Application_Model_Application_Abstract extends Core_Model_Default
{
    ...
    public function save()
    {
        ...
        if (!$this->getId()) {
            ...
            if (empty($applicationName)) {
                throw new \Nwicode\Exception(__('Name is required to save the Application.'));
            }
    ...
    public function setName($name)
    {
        ...
        if (is_numeric(substr($name, 0, 1))) {
            throw new \Nwicode\Exception(p__("application", "The application's name cannot start with a number"));
        }

        if (strlen($name) < 6) {
            throw new \Nwicode\Exception(p__("application", "The application's name must be at least 6 characters long."));
        }
    ...
    public function setDescription($description)
    {
        if (strlen($description) < 200) {
            throw new \Nwicode\Exception(p__("application","The description must be at least 200 characters"));
        }
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($privacyPolicy);
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($id);
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($key);
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($consumerKey);
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($consumerSecret);
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($apiToken);
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($apiSecret);
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($clientId);
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($token);
    ...
    {
        $_filtered = \Nwicode\Xss::sanitize($secret);
    ...
    public function setAppmetricaKey($key)
    {
        $_filtered = \Nwicode\Xss::sanitize($key);

        return $this->setData('appmetrica_key', $_filtered);
    }
    public function setAppmetricaAppId($key)
    {
        $_filtered = \Nwicode\Xss::sanitize($key);

        return $this->setData('appmetrica_app_id', $_filtered);
    }
    public function setOauthYandexToken($key)
    {
        $_filtered = \Nwicode\Xss::sanitize($key);

        return $this->setData('oauth_yandex_token', $_filtered);
    }
    ...
    public function buildId($type = "app")
    {
        ...
        if (Nwicode::getWhitelabel() !== false) {

            $whitelabel = Nwicode::getWhitelabel();
    ...
    public function checkCustomerAccount ()
    {
        ...
        if ($useMyAccount) {
            ...
            if (!$option->getId()) {
                throw new \Nwicode\Exception(__("My account feature is missing!"));
            }
    ...
    {
        if (Nwicode_Version::TYPE != "PE") return true;
    ...
    {
        if (Nwicode_Version::TYPE != "PE") return false;
    ...
    public function toYml()
    {
        ...
        $dataset = Nwicode_Yaml::encode($dataset);
    ...
    public function wipe()
    {
            ...
            if ($optionValue->getId()) {
                ...
                if ($mcommerce->getId()) {
                    ...
                    if ($mcommerceOrder->count() > 0) {
                        throw new Nwicode_Exception(__('This Application has some M-Commerce invoices associated, you can not delete it!'));
                    }
                }
            }
        }
        ...
        if (strlen($appId) <= 0) {
            throw new Nwicode_Exception(__('Seems the appId is empty or invalid, aborting!'));
        }

        $pathToImages = Core_Model_Directory::getBasePathTo('/images/application/' . $appId . '/');
        if (strrpos($pathToImages, '//') === 0) {
            throw new Nwicode_Exception(__('Seems the computed path could delete unwanted content, aborting!<br />' . $pathToImages));
        }
        ...
        if ($absolutePath === $applicationAbsolutePath) {
            throw new Nwicode_Exception(__('Seems we could delete unwanted files, aborting!'));
        }
