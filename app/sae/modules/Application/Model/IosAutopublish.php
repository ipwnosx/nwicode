...
class Application_Model_IosAutopublish extends Core_Model_Default
{
     ...
     * @var string
     */
    public $autopublishApiEndpoint = 'https://autopublish-api.nwicodecms.com/get-teams';
    ...
    {
        $teams = Nwicode_Json::decode($this->getData('teams'));
    ...
    public function getTeamsArray ()
    {
        ...
        $dataTeams = [];
        if (!empty($teams)) {
            $teams = Nwicode_Json::decode($teams);
            foreach ($teams['teams'] as $team) {
                $dataTeams[] = [
                    'teamId' => $team['teamId'],
                    'type' => $team['type'],
    ...
    public function getStats ()
    {
        try {
             ...
             * }
             */
            $response = Nwicode_Request::get('https://autopublish-api.nwicodecms.com/stats');
            $values = Nwicode_Json::decode($response);
