...
class Application_Model_SourceQueue extends Core_Model_Default
{
    ...
    protected function sendJobToAutoPublishServer($application, $sourcePath)
    {
        ...
        $languages = Nwicode_Json::decode($appIosAutopublish->getLanguages());

        if (!array_key_exists('error', $languages)) {
            if (count($languages) === 0) {
                throw new \Nwicode\Exception('There is no language selected');
            }
        } else {
            throw new \Nwicode\Exception('Cannot unserialize language data');
        }

        //we keep using ISO-639 for nwicode storage but we have to translate ISO code to faslane languages name
        $fastlane_languages = [];
        ...
        if (!mkdir($jobFolder, 0777, true)) {
            throw new \Nwicode\Exception('Cannot create folder ' . $jobFolder);
        }

        if (!copy($sourcePath, $jobFolder . '/sources.zip')) {
            throw new \Nwicode\Exception('Cannot copy sources to job folder');
        }
        ...
        if (!array_key_exists("error", Json::decode($json))) {
            ...
            throw new \Nwicode\Exception("Cannot create json config job file");
        }
        ...
        if ($return_val !== 0) {
            throw new \Nwicode\Exception('Cannot create zip job file');
        }
        ...
        Nwicode_Request::get(
            ...
            [
                ...
                'NWICODE_JOB_URL' => $jobUrlEncoded,
                'VERSION' => "3",
            ]);

        if (!in_array(Nwicode_Request::$statusCode, [100, 200, 201])) {
            throw new \Nwicode\Exception(__('Cannot send build to service %s.', Nwicode_Request::$statusCode));
        }
    }
    ...
    protected function sendPemToAutoPublishServer($application)
    {
        ...
        $configJobFilePath = $jobFolder . '/config.json';
        $json = Nwicode_Json::encode($data, JSON_PRETTY_PRINT);
