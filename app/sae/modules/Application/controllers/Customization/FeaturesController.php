...
//                    'message' => $e->getMessage(),
        ...
        if ($datas = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                if (empty($datas['option_value_id']) OR empty($datas['tabbar_name'])) {
                    throw new Nwicode_Exception(__('An error occurred while saving your page name.'));
                }
        ...
        if ($optionValue->getCode()) {
            ...
            if (!$option->getId()) {
                ...
                if (Nwicode_Exporter::isRegistered($option->getCode())) {
                    $classname = Nwicode_Exporter::getClass($option->getCode());
    ...
    public function unlockfeaturesAction() {
        $layout = $this->getLayout();
        $layout->setBaseRender('modal', 'html/modal.phtml', 'core_view_default')
            ->setTitle(__('Unlock features'))
            ->setBorderColor("border-blue")
        ;
        $layout->addPartial('modal_content', 'admin_view_default', 'application/customization/features/unlockfeatures.phtml');
        ...
        if ($application_form_export->isValid($request->getParams())) {
            ...
            foreach ($options as $option) {
                if (isset($selected_options[$option->getId()]) && $selected_options[$option->getId()]) {
                    if (Nwicode_Exporter::isRegistered($option->getCode())) {
                        $exporter_class = Nwicode_Exporter::getClass($option->getCode());
            ...
            $package = [
                ...
                "flavor" => Nwicode_Exporter::FLAVOR,
                "type" => "template",
                "dependencies" => [
                    "system" => [
                        "type" => "SAE",
                        "version" => Nwicode_Exporter::MIN_VERSION,
                    ],
                ],
            ];

            file_put_contents("{$tmp_directory}/package.json", Nwicode_Json::encode($package));
