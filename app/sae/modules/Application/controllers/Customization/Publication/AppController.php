...
class Application_Customization_Publication_AppController extends Application_Controller_Default
{
    ...
    public function iconspostAction()
    {
        ...
        if ($form->isValid($values)) {
            ...
            if (isset($values["icon"]) && !file_exists(Core_Model_Directory::getBasePathTo("images/application" . $values["icon"]))) {
                $path_icon = Nwicode_Feature::moveUploadedIcon($application->getId(), Core_Model_Directory::getTmpDirectory() . "/" . $values['icon']);
            ...
            if (isset($values["android_push_icon"]) && !file_exists(Core_Model_Directory::getBasePathTo("images/application" . $values["android_push_icon"]))) {
                $path_android_push_icon = Nwicode_Feature::moveUploadedIcon($application->getId(), Core_Model_Directory::getTmpDirectory() . "/" . $values['android_push_icon']);
            ...
            if (isset($values["android_push_image"]) && $values["android_push_image"] == "_delete_") {
                ...
                $path_android_push_image = Nwicode_Feature::moveUploadedIcon($application->getId(), Core_Model_Directory::getTmpDirectory() . "/" . $values['android_push_image']);
    ...
    {
        $this->getLayout()->setBaseRender('content', 'application/customization/publication/app/tour.phtml', 'admin_view_default');
        $html = ['html' => $this->getLayout()->render()];
        $this->getLayout()->setHtml(Zend_Json::encode($html));
    }
    ...
    public function touruidupdateAction() {
        $application = $this->getApplication();
        $application->setData('tour_uid',time())->save();
        $payload = [
            'success' => '1',
    ...
    public function tourpostAction()
    {
        ...
        if ($datas = $this->getRequest()->getPost()) {
            ...
            try {
                $application = $this->getApplication();
                $relative_path = '/' . $application->getId() . '/tour_images/';
                $folder = Application_Model_Application::getBaseImagePath() . $relative_path;
                $application
                    ->setData('enable_tour',$datas['enable_tour'])
                    ->setData('tour_title_color_type',$datas['tour_title_color_type'])
                    ->setData('tour_subtitle_color_type	',$datas['tour_subtitle_color_type'])
                    ->setData('tour_title_color	',$datas['tour_title_color'])
                    ->setData('tour_subtitle_color',$datas['tour_subtitle_color'])
                    ->setData('tour_title_1',$datas['tour_title_1'])
                    ->setData('tour_title_2',$datas['tour_title_2'])
                    ->setData('tour_title_3',$datas['tour_title_3'])
                    ->setData('tour_title_4',$datas['tour_title_4'])
                    ->setData('tour_title_5',$datas['tour_title_5'])
                    ->setData('tour_subtitle_1',$datas['tour_subtitle_1'])
                    ->setData('tour_subtitle_2',$datas['tour_subtitle_2'])
                    ->setData('tour_subtitle_3',$datas['tour_subtitle_3'])
                    ->setData('tour_subtitle_4',$datas['tour_subtitle_4'])
                    ->setData('tour_subtitle_5',$datas['tour_subtitle_5'])
                    ->setData('tour_header',$datas['tour_header'])
                    ->setData('tour_tbs',$datas['tour_tbs'])
                    ->setData('tour_lbt',$datas['tour_lbt'])
                    ->setData('tour_tbc',$datas['tour_tbc'])
                    ->setData('tour_uid',time())
                    ->save();

                //images
                for($i=1;$i<=5;$i++) {
                    if ($datas['tour_slide_'.$i.'_img_remove']=="1") {
                        $application->setData('tour_slide_'.$i,"");
                    } else if ($datas['tour_slide_'.$i.'_img_new']=="1"){
                        //copy
                        $file = Core_Model_Directory::getTmpDirectory(true).'/'.$datas['tour_slide_'.$i.'_img'];  
                        if (!is_dir($folder)) {
                            mkdir($folder, 0777, true);
                        }
                        $datas['folder']=$folder;
                        $datas['file']=$file;
                        if (!copy($file, $folder.$datas['tour_slide_'.$i.'_img'])) {
                            throw new exception(__('An error occurred while saving. Please try again later.'));
                        } else {
                            $application->setData('tour_slide_'.$i, $relative_path.$datas['tour_slide_'.$i.'_img']);
                        }
