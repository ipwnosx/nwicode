...
class Application_Backoffice_IosautopublishController extends Backoffice_Controller_Default
{
    ...
    public function savecredentialsAction ()
    {
        try {
            ...
            if (empty($params)) {
                throw new \Nwicode\Exception("#325-01: " . __("Missing parameters."));
            }

            if (empty($params["app_id"])) {
                throw new \Nwicode\Exception("#325-02: " . __("App Id is required!"));
            }

            if ((empty($ios["itunes_login"]) || empty($ios["itunes_password"]))) {
                throw new \Nwicode\Exception("#325-03: " . __("Please fill App Store Connect Credentials."));
            }

            if (empty($ios["selected_team"]) || empty($ios["selected_team_name"]) || empty($ios["selected_provider"])) {
                throw new \Nwicode\Exception("#325-04: " . __("Please fill Dev team & Provider."));
            }
    ...
    public function requestpublicationAction()
    {
        try {
            $request = $this->getRequest();
            $params = Nwicode_Json::decode($request->getRawBody());

            if (empty($params)) {
                throw new \Nwicode\Exception("#330-01: " . __("Missing parameters."));
            }

            if (empty($params["app_id"])) {
                throw new \Nwicode\Exception("#330-02: " . __("App Id is required!"));
            }
            ...
            if (!$appIosAutopublish->getId()) {
                throw new \Nwicode\Exception("#330-03: " . __("No credentials found!"));
            }

            if (empty($params["infos"]["languages"])) {
                throw new \Nwicode\Exception("#330-04: " . __("Please select at least one language."));
            }
            ...
            if (!$application->getId()) {
                throw new \Nwicode\Exception("#330-05: " . __("Application not found!"));
            }

            // Find selected team!
            if (empty($appIosAutopublish->getTeamId()) ||
                empty($appIosAutopublish->getItcProvider())) {
                throw new \Nwicode\Exception("#330-06: " .
            ...
            $appIosAutopublish
                ...
                ->setLanguages(Nwicode_Json::encode([
...
#
    ...
    public function uploadapkAction ()
    {
        ...
        try {
            ...
            if ($appId === false) {
                throw new \Nwicode\Exception('#565-04: ' . __('Missing appId'));
            }

            if (empty($_FILES) || empty($_FILES['file']['name'])) {
                throw new \Nwicode\Exception('#565-02: ' . __('No file has been sent'));
            }
            ...
            if ($adapter->receive()) {
                ...
                throw new \Nwicode\Exception($message);
            }
        } catch (\Exception $e) {
            $serviceQueue = Application_Model_SourceQueue::getApkServiceQueue($appId);
            $serviceQueue
    ...
    public function apkservicestatusAction ()
    {
        ...
        try {
            $appId = $request->getParam('appId', false);
            if ($appId === false) {
                throw new \Nwicode\Exception('#566-01: ' . __('Missing appId'));
            }

            $status = $request->getParam('status', false);
            if ($status === false) {
                throw new \Nwicode\Exception('#566-02: ' . __('Missing status'));
            }
    ...
    public function updatejobstatusAction() {
        ...
        try {
            ...
            if (is_null($token) || is_null($status)) {
                throw new \Nwicode\Exception(__('Missing token and/or status.'));
            }
            ...
            if (!in_array($status, $availableStatuses)) {
                throw new \Nwicode\Exception(__('Invalid status `%s`.', $status));
            }
            ...
            if (!$appIosAutopublish->getId()) {
                throw new \Nwicode\Exception(__('Unable to find the corresponding build.'));
            }
            ...
            switch ($status) {
                case 'success':
                    ...
                    if (!$application->getId()) {
                        throw new \Nwicode\Exception(__('Cannot find application from token.'));
                    }
                    //1 is iOS
