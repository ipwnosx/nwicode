...
class Application_Backoffice_ViewController extends Backoffice_Controller_Default
{
    ...
    public function findAction()
    {
        ...
        $languages = 'en';
        if ($lang = Nwicode_Json::decode($appIosAutopublish->getLanguages())) {
    ...
    public function switchionicAction()
    {
        if ($data = Zend_Json::decode($this->getRequest()->getRawBody())) {
            ...
            try {
                ...
                if (!$application->getId()) {
                    throw new Nwicode_Exception(__("Application id %s not found.", $data["app_id"]));
                }
    ...
    public function downloadsourceAction()
    {
        ...
        try {
            ...
            if (empty($params)) {
                throw new \Nwicode\Exception(__('Missing parameters for generation.'));
            }

            $application = new Application_Model_Application();

            if (empty($params['app_id']) OR empty($params['device_id'])) {
                throw new \Nwicode\Exception('#908-00: ' . __('This application does not exist'));
            }

            $application->find($params['app_id']);
            if (!$application->getId()) {
                throw new \Nwicode\Exception('#908-01: ' . __('This application does not exist'));
            }

            $mainDomain = __get('main_domain');
            if (empty($mainDomain)) {
                throw new \Nwicode\Exception('#908-02: ' .
            ...
            if (!Cron_Model_Cron::is_active()) {
                ...
                $nwicode_cron = new Nwicode_Cron();
                $nwicode_cron->execute($task);
                $reload = true;
            }
