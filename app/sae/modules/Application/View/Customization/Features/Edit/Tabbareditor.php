...
class Application_View_Customization_Features_Edit_Tabbareditor extends Application_View_Customization_Features_List_Options {
    ...
    protected function _calcIconSize() {
        ...
        if(!$current_option->getFolderCategoryId() && Nwicode_Feature::getRatioCallback($layout->getCode())) {
            $callback = Nwicode_Feature::getRatioCallback($layout->getCode());
            ...
            $sizes = call_user_func_array($callback, array($actual_position, Nwicode_Json::decode($application->getLayoutOptions())));
