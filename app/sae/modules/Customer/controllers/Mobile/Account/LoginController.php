...
class Customer_Mobile_Account_LoginController extends Application_Controller_Mobile_Default
{
    ...
    public function postv2Action()
    {
        try {
            ...
            if ($params = Nwicode_Json::decode($request->getRawBody())) {
                ...
                throw new \Nwicode\Exception(__("Missing parameters."));
            }
        } catch (\Exception $e) {
            $payload = [
                "error" => true,
    ...
    public function postAction()
    {
        ...
        \Nwicode\Hook::trigger('mobile.login', [
        ...
        try {
            if ($datas = Nwicode_Json::decode($request->getRawBody())) {

                if ((empty($datas['email']) || empty($datas['password']))) {
                    throw new Nwicode_Exception(
                ...
                \Nwicode\Hook::trigger('mobile.login.success', [
                    'appId' => $application->getId(),
                ...
                throw new Nwicode_Exception(__("An error occurred, please try again."));
            }
            ...
            \Nwicode\Hook::trigger('mobile.login.error', [
                'appId' => $application->getId(),
                'message' => $e->getMessage(),
                'type' => 'account'
            ]);
    ...
    public function loginwithfacebookAction()
    {
        ...
        $datas = Nwicode_Json::decode($this->getRequest()->getRawBody());
        if (isset($datas["token"])) {
            ...
            try {
                ...
                if ($access_token === false) {
                    throw new Nwicode_Exception(__('An error occurred while connecting to your Facebook account. Please try again later'));
                }
    ...
    public function logoutAction()
    {
        ...
        \Nwicode\Hook::trigger('mobile.logout', [
            'appId' => $application->getId(),
            'request' => $request
        ]);
        ...
        \Nwicode\Hook::trigger('mobile.logout.success', [
            'appId' => $application->getId(),
