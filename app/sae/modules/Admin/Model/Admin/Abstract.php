...
abstract class Admin_Model_Admin_Abstract extends Core_Model_Default
{
    ...
    public function setEmail($email)
    {
        ...
        if (!$validator->isValid($_filtered)) {
            throw new \Nwicode\Exception(__('This is not a valid e-mail address.'));
        }
    ...
    public function save()
    {
        ...
        if (empty($this->getEmail())) {
            throw new \Nwicode\Exception(__('E-mail is required!'));
        }

        if ($this->getCountryCode()) {
            if (empty($countries[$this->getCountryCode()])) {
                throw new \Nwicode\Exception(__("An error occurred while saving. The country is not valid."));
    ...
    public function setPassword($password)
    {
        if (strlen($password) < 6) {
            throw new Nwicode_Exception(__('The password must be at least 6 characters'));
        }
