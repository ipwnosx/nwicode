        ...
        if ($data = Zend_Json::decode($this->getRequest()->getRawBody())) {
            ...
            try {
                ...
                if (!empty($data["password"]) AND $data["password"] != $data["confirm_password"]) {
                    throw new Nwicode_Exception(__("Passwords don't match"));
                }

                $admin->addData($data);

                if ($dummy->getEmail() == $admin->getEmail() AND $dummy->getId() != $admin->getId()) {
                    throw new Nwicode_Exception(__("We are sorry but this email address already exists."));
                }
        ...
        if ($data = Zend_Json::decode($this->getRequest()->getRawBody())) {
            ...
            try {
                ...
                if (empty($data["admin_id"]) OR empty($data["app_id"])) {
                    throw new Nwicode_Exception(__("#103: An error occurred while saving. Please try again later."));
                }
                ...
                if (!$admin->getId() OR !$application->getId()) {
                    throw new Nwicode_Exception(__("#104: An error occurred while saving. Please try again later."));
                }
