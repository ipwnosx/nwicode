...
class Admin_AccountController extends Admin_Controller_Default
{
            ...
            try {
                ...
                if (__getConfig('is_demo')) {
                    if (in_array($data['email'], ['client@client.com', 'demo@demo.com'])) {
                        throw new \Nwicode\Exception(__('You are not allowed to edit this account in demo!'));
                    }
                }
                ...
                if (!empty($data['admin_id'])) {
                    ...
                    if (!$admin->getId()) {
                        throw new \Nwicode\Exception(__('There is no user with this ID.'));
                    }
                }
                ...
                if ($admin->getId() &&
                    ...
                    throw new \Nwicode\Exception(__("An error occurred while saving your account. Please try again later."));
                }
                ...
                if ($check_email_admin->getId() AND $check_email_admin->getId() != $admin->getId()) {
                    throw new \Nwicode\Exception(__('This email address is already used'));
                }

                if (isset($data['password'])) {
                    if ($data['password'] != $data['confirm_password']) {
                        throw new \Nwicode\Exception(__('Your password does not match the entered password.'));
                    }
                    if (!empty($data['old_password']) AND !$admin->isSamePassword($data['old_password'])) {
                        throw new \Nwicode\Exception(__("The old password does not match the entered password."));
                    }
                ...
                if (empty($data["role_id"]) && $data["mode"] == "management") {
                    throw new \Nwicode\Exception(__('The account role is required'));
                } else {
                ...
                if ($data['mode'] == 'management') {
                    ...
                    if (!$isAllowedRole) {
                        throw new \Nwicode\Exception(__("Your are not allowed to assign this role."));
                    }
                } else {
                    unset($data['role_id']);
                }
    ...
    public function sendmakeformeAction()
    {
        $request = $this->getRequest();
        $params = $request->getParams();
        $supportemail = System_Model_Config::getValueFor("support_email");
        $adminmail = $this->getAdmin()->getEmail();
        if (Nwicode_Version::TYPE === 'SAE'){
            $supportemail = "support@nwicode.com";
        }
        try {
            ...
            $mail->setBodyHtml($params["message"]);
            $mail->setFrom($adminmail);
            $mail->addTo($supportemail);
            $mail->send();

            $success_message = (__('Your request has been successfully sent'));
            $payload = [
                'success' => '1',
    ...
    public function deleteAction()
    {
        ...
        if ($admin_id = $this->getRequest()->getParam('admin_id') AND !$this->getSession()->getAdmin()->getParentId()) {
            ...
            try {
                ...
                if (__getConfig('is_demo')) {
                    if (in_array($admin->getEmail(), ['client@client.com', 'demo@demo.com'])) {
                        throw new \Nwicode\Exception(__('You are not allowed to delete this account in demo!'));
                    }
                }
    ...
    public function ionlineAction() {
        $current_adminid = $this->getAdmin()->getId();
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $sql = "UPDATE `admin` SET `is_online` = '1' WHERE `admin`.`admin_id` = ?;";
        $db->query($sql, $current_adminid);
        $this->_redirect('');
        return $this;
    }
    public function iofflineAction() {
        $current_adminid = $this->getAdmin()->getId();
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $sql = "UPDATE `admin` SET `is_online` = '0' WHERE `admin`.`admin_id` = ?;";
        $db->query($sql, $current_adminid);
        $this->_redirect('');
        return $this;
    }
