...
class Api_Service_ImageController extends Core_Controller_Default {
    ...
    public function proxyAction() {
        ...
        $params_raw = Nwicode_Json::decode($request->getRawBody());
        ...
        try {
            $data = Nwicode_Image::getForMobile(
                $request->getBaseUrl(),
                $params['resource'],
                $params['format'],
                $params['device_width'],
    ...
    public function thumbnailAction () {
        ...
        try {
            ...
            if (!file_exists($resource)) {
                ...
                if (!file_exists($resource)) {
                    throw new Nwicode_Exception(__('The given resource doesn\'t exists'));
                }
            }

            Nwicode_Image::enableForceCache();

            $infos = Nwicode_Image::getForMobile(
                $request->getBaseUrl(),
                $resource,
                'thumbnail',
                null,
            ...
            Nwicode_Image::disableForceCache();
