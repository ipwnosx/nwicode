...
/**
 * Class Push_Backoffice_GlobalController
 */
class Push_Backoffice_GlobalController extends Backoffice_Controller_Default
{
    ...
    public function sendAction()
    {
        ...
        try {
            ...
            if ($params = Nwicode_Json::decode($this->getRequest()->getRawBody())) {
                ...
                if (empty($params["title"]) || empty($params["message"])) {
                    throw new Nwicode_Exception(__("Title & Message are both required."));
                }

                if (empty($params["checked"]) && !$params["send_to_all"]) {
                    throw new Nwicode_Exception(__("Please select at least one application."));
                }
                ...
                throw new Nwicode_Exception(__("%s, No params sent.", "Push_Backoffice_Global::sendAction"));
            }
