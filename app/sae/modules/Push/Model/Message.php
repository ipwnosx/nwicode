...
class Push_Model_Message extends Core_Model_Default
{
                ...
                if (in_array($this->getTargetDevices(), ["ios", "all", "", null])) {
                    try {
                        ...
                        if (is_readable($ios_certificate) && is_file($ios_certificate)) {
                            $instance = new Push_Model_Ios_Message(new Nwicode_Service_Push_Apns(null, $ios_certificate));
                            $instance->setMessage($this);
                            $instance->push();
                        } else {
                            throw new \Nwicode\Exception("You must provide an APNS Certificate for the App ID: {$this->getAppId()}");
                        }
                    } catch (\Exception $e) {
                        $this->logger->info(sprintf("[CRON: %s]: " . $e->getMessage(), date("Y-m-d H:i:s")), "cron_push");
                        $this->_log("Nwicode_Service_Push_Apns", $e->getMessage());
                        $errors[] = $e->getMessage();
                    ...
                    $this->_log("Nwicode_Service_Push_Apns", "ios is not in the target list, skipping.");
                }
            }
            ...
            if ($type == 'android') {
                ...
                if (in_array($this->getTargetDevices(), ["android", "all", "", null])) {
                    try {
                        ...
                        $gcmInstance = null;
                        if (!empty($gcmKey)) {
                            $gcmInstance = new \Nwicode\CloudMessaging\Sender\Gcm($gcmKey);
                        }
                        ...
                        $fcmInstance = null;
                        if (!empty($fcmKey)) {
                            $fcmInstance = new \Nwicode\CloudMessaging\Sender\Fcm($fcmKey);
                        }else {
                            // Only FCM is mandatory by now!
                            throw new \Nwicode\Exception("You must provide FCM Credentials");
                        }
                        ...
                        $this->_log("Nwicode_Service_Push_Fcm", $e->getMessage());
                        $errors[] = $e->getMessage();
                    ...
                    $this->_log("Nwicode_Service_Push_Apns", "android is not in the target list, skipping.");
                }
            }
        }
    ...
    protected function _initInstances()
    {
        ...
        if (is_null($this->_instances)) {
            ...
            foreach ($this->_types as $device => $type) {
                if ($device == 'iphone') {
                    $this->_instances[$device] = new $type(new Nwicode_Service_Push_Apns(ApnsPHP_Push::ENVIRONMENT_SANDBOX));
                } else {
    ...
    public function exportAction($option, $export_type = null)
    {
        if ($option && $option->getId()) {
            ...
            try {
                $result = Nwicode_Yaml::encode($dataset);
            } catch (\Exception $e) {
                throw new \Nwicode\Exception("#089-03: An error occured while exporting dataset to YAML.");
            }

            return $result;

        } else {
            throw new \Nwicode\Exception("#089-01: Unable to export the feature, non-existing id.");
        }
    }
    ...
    public function importAction($path)
    {
        ...
        try {
            $dataset = Nwicode_Yaml::decode($content);
        } catch (\Exception $e) {
            throw new \Nwicode\Exception("#089-04: An error occured while importing YAML dataset '$path'.");
        }
        ...
        if (isset($dataset["option"])) {
            ...
            throw new \Nwicode\Exception("#089-02: Missing option, unable to import data.");
        }
    }
}
