...
class Payment_Model_Paypal extends Payment_Model_Abstract
{
    ...
    public function __construct($user = false, $pwd = false, $signature = false, $isTesting = false)
    {
        ...
        if ($user && $pwd && $signature) {
            ...
            if (!$this->__user || !$this->__pwd || !$this->__signature) {
                throw new \Nwicode\Exception('Error, Paypal is not properly set up.', 100);
            }
        }
        ...
        try {
            ...
            if ($result['STATUS'] === 'Active') {
                ...
                if ($result['ACK'] !== 'Success') {
                    throw new \Nwicode\Exception(__('Unable to cancel PayPal subscription, tray again later.'));
                }
            } else {
        ...
        try {
            ...
            if ($result['ACK'] === 'Success') {
                if ($result['STATUS'] === 'Active') {
                    ...
                    throw new \Nwicode\Exception(__('PayPal subscription is %s.', $result['STATUS']));
                }
            } else if ($result['ACK'] === 'Failure') {
                throw new \Nwicode\Exception(__($result['L_LONGMESSAGE0']));
            }
        ...
        try {
            if ($order = $this->getOrder()) {
                if ($token = $this->getToken()) {
                    if (!$order->getId()) {
                        throw new \Nwicode\Exception(__('An error occurred while processing your order. Please, try again later.'));
                    }
                    ...
                    if ($paymentIsOk) {
                        ...
                        throw new \Nwicode\Exception(__('An error occurred while processing the payment. For more information, please feel free to contact us.'));
                    }
                } else {
                    throw new \Nwicode\Exception(__("An error occurred while processing your order. Please, try again later."));
                }
    ...
    /**
     * @param Nwicode_Cron $cronInstance
     * @param Subscription_Model_Subscription_Application $subscription
     * @throws Exception
     ...
     * @throws Zend_Exception
     */
    public static function checkRecurrencies(Nwicode_Cron $cronInstance, Subscription_Model_Subscription_Application $subscription)
    {
            ...
            switch ($frequency) {
                case 'Monthly':
                    if (!$saleModel->isInvoiceExistsForMonth(
                        ...
                        // indeed some nwicode already fix there accounting before
