            ...
            try {
                ...
                foreach ($blocks as $k => $block) {
                    ...
                    if($block["type"] == "address" AND (empty($block["latitude"]) OR empty($block["longitude"]))) {
                        $latlon = Nwicode_Google_Geocoding::getLatLng([
    ...
    public function addblockAction() {
        ...
        if ($datas = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                if (empty($position)) {
                    throw new Nwicode_Exception("#476-01: ".__('An error occurred during process. Please try again later.'));
                }
                ...
                if (!$block->getId()) {
                    throw new Nwicode_Exception("#476-02: ".__('An error occurred during process. Please try again later.'));
                }
