...
class Cms_Model_Application_Page extends Core_Model_Default
{
    ...
    public function edit_v2($option_value, $datas)
    {
        ...
        if (empty($datas['orderUniqid'])) {
            throw new \Nwicode\Exception('#578-21' . __('At least one section is required to save.'));
        }
        ...
        try {
            if (!$option_value) {
                throw new \Nwicode\Exception('#578-01' . __('An error occurred while saving your page.'));
            }
            ...
            if (!isset($datas['page_id']) || empty($datas['page_id']) || ($datas['page_id'] == 'new')) {
                ...
                if ($page->getId() && ($page->getValueId() != $value_id)) {
                    throw new \Nwicode\Exception('#578-02' . __('An error occurred while saving your page.'));
                }
            }
            ...
            foreach ($positions as $uniqid) {
                ...
                switch ($block_type) {
                    ...
                    case 'cover':
                        ...
                        break;
                    default:
                        throw new \Nwicode\Exception(__("This block type doesn't exists."));
                }
    ...
    {
        $picture = Nwicode_Feature::saveImageForOptionDelete($optionValue, $datas['places_file']);
        $thumbnail = Nwicode_Feature::saveImageForOptionDelete($optionValue, $datas['places_thumbnail']);
        $pin = Nwicode_Feature::saveImageForOptionDelete($optionValue, $datas['places_pin']);
