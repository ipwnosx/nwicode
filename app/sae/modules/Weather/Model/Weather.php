...
class Weather_Model_Weather extends Core_Model_Default
{
    ...
    {
        $request = \Nwicode_Request::get("https://api.openweathermap.org/data/2.5/weather?q=London&appid={$key}");
        $result = \Nwicode_Json::decode($request);

        // Transfer error code if cod match 40x
        if (strpos($result["cod"], "40") === 0) {
            throw new \Nwicode\Exception(__("OpenWeatherMap") . "<br />" . $result["message"]);
        }

        if (\Nwicode_Request::$statusCode != "200") {
            throw new \Nwicode\Exception(__(__("OpenWeatherMap") . "<br />" . "We were unable to communicate with the OpenWeatherMap API! Please try again later!"));
        }
    }
