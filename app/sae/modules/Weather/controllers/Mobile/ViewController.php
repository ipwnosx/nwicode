...
class Weather_Mobile_ViewController extends Application_Controller_Mobile_Default
{
    ...
    public function findAction()
    {
        try {
            ...
            if (!$valueId) {
                throw new \Nwicode\Exception(__("An error occurred during process. Please try again later."));
            }
    ...
    public function getweatherAction()
    {
        try {
            ...
            if (empty($params)) {
                throw new \Nwicode\Exception(__("Missing params"));
            }
            ...
            if (!$result) {
                ...
                $weather = Nwicode_Request::get(self::$owmCurrentEndpoint, $data);
                $responseWeather = Nwicode_Json::decode($weather);

                // Transfer error code if cod match 40x
                if (strpos($responseWeather["cod"], "40") === 0) {
                    throw new \Nwicode\Exception($responseWeather["message"]);
                }

                $forecast = Nwicode_Request::get(self::$owmForecastEndpoint, $data);
                $responseForecast = Nwicode_Json::decode($forecast);

                // Transfer error code if cod match 40x
                if (strpos($responseForecast["cod"], "40") === 0) {
                    throw new \Nwicode\Exception($responseForecast["message"]);
                }
