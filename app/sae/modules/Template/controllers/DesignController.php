...
class Template_DesignController extends Application_Controller_Default
{
    ...
    public function saveAction()
    {
        try {
            ...
            if (empty($datas)) {
                throw new \Nwicode\Exception(__('#156: Missing params!'));
            }

            if (empty($datas['design_id'])) {
                throw new \Nwicode\Exception(__('#118: An error occurred while saving'));
            }
            ...
            if (!$design->getId()) {
                throw new \Nwicode\Exception(__('#119: An error occurred while saving'));
            } else if ($design->getCode() != "blank" && empty($datas['category_id'])) {
                throw new \Nwicode\Exception(__('#120: An error occurred while saving'));
            }

            if (!empty($datas['category_id'])) {
                $category->find($datas['category_id']);
                if (!$category->getCode()) {
                    throw new \Nwicode\Exception(__('#121: An error occurred while saving'));
                }
            }
