...
class Template_Model_Db_Table_Block extends Core_Model_Db_Table {
    ...
    public function findAll($values, $order, $params) {
        ...
        if(!empty($values['app_id'])) {
			...
			// Ionic4 fields
			 $fields['ion_color'] = new Zend_Db_Expr('IFNULL(tba.ion_color, tb.ion_color)');
			 $fields['ion_color_contrast'] = new Zend_Db_Expr('IFNULL(tba.ion_color_contrast, tb.ion_color_contrast)');
			 $fields['ion_color_shade'] = new Zend_Db_Expr('IFNULL(tba.ion_color_shade, tb.ion_color_shade)');
			 $fields['ion_color_tint'] = new Zend_Db_Expr('IFNULL(tba.ion_color_tint, tb.ion_color_tint)');
    ...
    public function findByDesign($design_id) {
        ...
        $select = $this->select()
            ...
            ->join(['tdb' => 'template_design_block'], 'tdb.design_id = td.design_id', ['block_id', 'color', 'background_color', 'border_color', 'image_color', 'text_opacity', 'background_opacity', 'border_opacity', 'image_opacity','ion_color','ion_color_contrast','ion_color_shade','ion_color_tint'])
