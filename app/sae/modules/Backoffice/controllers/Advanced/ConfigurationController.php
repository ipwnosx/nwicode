...
class Backoffice_Advanced_ConfigurationController extends System_Controller_Backoffice_Default
{
    ...
    public function findallAction()
    {
        ...
        $result = Nwicode_Network::testSsl($this->getRequest()->getHttpHost(), true);
        $data["testssl"] = $result;
        ...
        foreach ($certs as $cert) {
            ...
            $cert_data = [
                "id" => $cert->getId(),
                "whitelabels" => $wls,
                "domains" => Nwicode_Json::decode($cert->getDomains()),
    ...
    public function submitreportAction()
    {
        try {
            ...
            if ($params = Nwicode_Json::decode($request->getRawBody())) {
                ...
                $request->post(sprintf("http://nwicode.com/report.php?type=%s", Nwicode_Version::TYPE), $bug_report);
                ...
                throw new Nwicode_Exception(__("Message is required."));
            }
        ...
        if ($cert_id = $this->getRequest()->getParam("cert_id")) {
            try {
                ...
                if (__getConfig('is_demo') && in_array($cert->getHostname(), __getConfig('hostname'))) {
                    throw new Nwicode_Exception('This certificate is protected.');
                }
            ...
            try {
                ...
                if ($data["upload"] == "1") {
                    ...
                    $ssl_cert
                        ...
                        ->setDomains(Nwicode_Json::encode($certificate_hosts))
                    ...
                    # Test if all three paths are readable from Nwicode !!!
    ...
    {
        $result = Nwicode_Network::testSsl($this->getRequest()->getHttpHost());
    ...
    public function clearpleskAction()
    {
        try {
            ...
            $nwicode_plesk = new Nwicode_Plesk();
            $nwicode_plesk->removeCertificate($cert);
    ...
    public function installpleskAction()
    {
        try {
            ...
            $nwicode_plesk = new Nwicode_Plesk();
            $nwicode_plesk->updateCertificate($cert);
    ...
    public function sendtopanelAction()
    {
        ...
        try {
            ...
            switch ($panel_type) {
                case "plesk":
                    $nwicode_plesk = new Nwicode_Plesk();
                    $nwicode_plesk->selectCertificate($cert);

                    break;
                case "cpanel":
                    $cpanel = new Nwicode_Cpanel();
                    $cpanel->updateCertificate($cert);

                    break;
                case "vestacp":
                    $vestacp = new Nwicode_VestaCP();
                    $vestacp->updateCertificate($cert);

                    break;
                case "directadmin":
                    $directadmin = new Nwicode_DirectAdmin();
    ...
    public function generatesslAction()
    {
        ...
        try {
            ...
            if (($letsencrypt_disabled > time()) && ($letsencrypt_env === "production")) {
                ...
                throw new Nwicode_Exception(__("Certificate renewal is disabled until %s due to rate limit hit.", datetime_to_format(date("Y-M-d H:i:s", $letsencrypt_disabled))));
            ...
            $panel_type = System_Model_Config::getValueFor("cpanel_type");
            if ($panel_type == "-1") {
                throw new Nwicode_Exception(__("You must select an Admin panel type before requesting a Certificate."));
            }
            ...
            if (in_array($panel_type, ["cpanel", "plesk", "vestacp", "directadmin"])) {
                ...
                $proxy01 = "http://proxy01.nwicodecms.com/acme-challenge.php";
                $query = [
                    "secret" => Core_Model_Secret::SECRET,
                    "type" => Nwicode_Version::TYPE,
            ...
            $lets_encrypt = new Nwicode_LetsEncrypt($base, $root, $logger);
            ...
            if ($result) {
                ...
                $cert
                    ...
                    ->setDomains(Nwicode_Json::encode(array_values($hostnames), JSON_OBJECT_AS_ARRAY))
                    ->save();
