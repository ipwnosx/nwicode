...
class Backoffice_Advanced_CronController extends Backoffice_Controller_Default
{
    ...
    public function findallAction()
    {
        ...
        foreach ($cron_tasks as $cron_task) {
            ...
            $schedule = \Nwicode\Cron\Human::fromCronString($cronString);
        ...
        foreach ($apk_queue as $apk) {
            ...
            $_data["stuck"] = (($_data["status"] == "building") && (($_data["build_start_time"] + Nwicode_Date::HOUR_SECONDS) < time()));
    ...
    public function restartApkAction()
    {
        try {
            ...
            if (!$apkQueue->getId()) {
                throw new \Nwicode\Exception(__('This build do not exists.'));
            }
