...
class Front_MobileController extends Application_Controller_Mobile_Default
{
    ...
    public function loadv3Action()
    {
        ...
        if (!$result = $this->cache->load($cache_id_loadv2)) {
            ...
            $homepage_image_b64 = Nwicode_Image::open($homepage_image)->cropResize(256)->inline();
            ...
            $progressbar_color = Nwicode_Color::newColor($progressbar_color, "hex");
            $progressbar_trail_color = Nwicode_Color::newColor($progressbar_trail_color, "hex");
            ...
            $bg_color = Nwicode_Color::newColor($bg_color_hex, "hex");
            $bg_color->alpha = $bg_block->getBackgroundOpacity() / 100;

            $colorStatusBar = Nwicode_Color::newColor($application->getBlock("header")->getBackgroundColor(), 'hex');
        ...
        if (!$result = $this->cache->load($cache_id_homepage)) {
            ...
            if (!empty($layout_options) && $opts = Nwicode_Json::decode($layout_options)) {
                $layout_options = $opts;
            } else {
        ...
        if ($customer_id) {
            ...
            if (Nwicode_CustomerInformation::isRegistered("stripe")) {
                $exporter_class = Nwicode_CustomerInformation::getClass("stripe");
    ...
    public function generatewebappconfig($refresh = false)
    {
        ...
        $app_icon_base64 = Nwicode_Image::open(Core_Model_Directory::getBasePathTo($app_icon))->scaleResize(192, 192);
        $app_icon_144_base64 = Nwicode_Image::open(Core_Model_Directory::getBasePathTo($app_icon))->scaleResize(144, 144);
        $app_icon_512_base64 = Nwicode_Image::open(Core_Model_Directory::getBasePathTo($app_icon))->scaleResize(512, 512);
        $startup_image_base64 = Nwicode_Image::open(Core_Model_Directory::getBasePathTo($application->getStartupImageUrl()))->jpeg();
        ...
        if (!is_readable($manifest_name_base) || $refresh) {
            ...
            file_put_contents($manifest_name_base, Nwicode_Json::encode($manifest));
        }
    ...
    public function backgroundimagesAction()
    {
        try {
            ...
            // Homepage global!
            try {
                $backgrounds['home'] = Nwicode_Image::getForMobile(
                    $base_url,
                    Core_Model_Directory::getBasePathTo($application->getHomepageBackgroundImageUrl($device_resolution))
                );
                $backgrounds['landscape_home'] = Nwicode_Image::getForMobile(
                    $base_url,
            ...
            foreach ($options as $option) {
                ...
                if ($option->hasBackgroundImage() &&
                    ...
                    try {
                        $background = Nwicode_Image::getForMobile(
                            $base_url,
                    ...
                    try {
                        $landscape_background = Nwicode_Image::getForMobile(
                            $base_url,
            ...
            if (Nwicode_Debug::isDevelopment()) {
                $payload['debug'] = [
