...
class Front_Controller_Api_Base extends Front_Controller_App_Default
{
    ...
    public function _loadBlock ($application)
    {
        ...
        if (!$result = $this->cache->load($cacheId)) {
            ...
            $homepageImageB64 = Nwicode_Image::open($homepageImage)
            ...
            $progressbarColor = Nwicode_Color::newColor($progressbarColor, 'hex');
            $progressbarTrailColor = Nwicode_Color::newColor($progressbarTrailColor, 'hex');
            ...
            $bgColor = Nwicode_Color::newColor($bgColorHex, 'hex');
    ...
    public function _customerBlock ($application, $loadBlock)
    {
        ...
        if ($customerId) {
            ...
            if (Nwicode_CustomerInformation::isRegistered('stripe')) {
                $exporterClass = Nwicode_CustomerInformation::getClass('stripe');
    ...
    public function _manifestBlock($application, $request, $refresh = false)
    {
        ...
        $appIconBase64 = Nwicode_Image::open(Core_Model_Directory::getBasePathTo($appIcon))
            ->scaleResize(192, 192);
        $appIcon144Base64 = Nwicode_Image::open(Core_Model_Directory::getBasePathTo($appIcon))
            ->scaleResize(144, 144);
        $appIcon512Base64 = Nwicode_Image::open(Core_Model_Directory::getBasePathTo($appIcon))
            ->scaleResize(512, 512);
        ...
        if (!is_readable($manifestMameBase) || $refresh) {
            ...
            file_put_contents($manifestMameBase, Nwicode_Json::encode($manifest));
        }
