                            ...
                            if (!empty($data[$field->getId()])) {
                                ...
                                if ($field->getType() == "image") {
                                    ...
                                    if (!preg_match("@^data:image/([^;]+);@", $image, $matches)) {
                                        throw new \Nwicode\Exception(__("Unrecognized image format"));
                                    }

                                    $extension = strtolower($matches[1]);

                                    if (!in_array($extension, ["jpg", "jpeg", "png", "gif", "bmp"])) {
                                        throw new \Nwicode\Exception(__("Forbidden image format"));
                                    }
                                    ...
                                    if ($contents === FALSE) {
                                        throw new \Nwicode\Exception(__("No uploaded image"));
                                    }

                                    $res = file_put_contents($filePath, $contents);
                                    if ($res === FALSE) {
                                        throw new \Nwicode\Exception('Unable to save image');
                                    }
                                    ...
                                    $finalPath = Nwicode_Feature::moveUploadedFile($this->getCurrentOptionValue(), $filePath);
