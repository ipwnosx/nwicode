        ...
        try {
            ...
            if ($formDatas = $request->getPost()) {
                ...
                if (!$currentOption->getId()) {
                    throw new Nwicode_Exception(__('Missing value_id!'));
                }
                ...
                if (key_exists('param_instagram', $formDatas) && !empty($formDatas['param_instagram'])) {
                    if (!$app->getInstagramClientId() || !$app->getInstagramToken()) {
                        throw new Nwicode_Exception(__("Instagram API settings can't be found."));
                    }

                    $userId = (new Media_Model_Gallery_Image_Instagram())->getUserId();
                    if (!$userId) {
                        throw new Nwicode_Exception(__('The entered name is not a valid Instagram user.'));
                    }
                    ...
                    if(!$app->getFlickrKey() || !$app->getFlickrSecret()) {
                        throw new Nwicode_Exception(__("Flickr API settings can't be found."));
                    }
                    ...
                    try {
                        ...
                        throw new Nwicode_Exception(__('No valid Facebook API key and secret.'));
                    }
                ...
                throw new Nwicode_Exception(__('Missing form data!'));
            }
