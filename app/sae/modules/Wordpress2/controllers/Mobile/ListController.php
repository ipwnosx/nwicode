...
class Wordpress2_Mobile_ListController extends Application_Controller_Mobile_Default
{
    ...
    public function findAction()
    {
        try {
            ...
            if (!$wordpress->getId()) {
                throw new Nwicode_Exception('#33-001: ' . __('An error occured.'));
            }
            ...
            if (!$result || $refresh) {
                ...
                foreach ($wordpressQueries as $wordpressQuery) {
                    $query = Nwicode_Json::decode($wordpressQuery->getData('query'));
                ...
                $this->cache->save(Nwicode_Json::encode($payload), $cacheId, [
                    'wordpress2',
                    'findAction',
                    'value_id_' . $valueId,
                    $cacheTag
                ...
                $payload = Nwicode_Json::decode($result);
                $payload['x-cache'] = 'HIT';
            }
    ...
    public function loadpostsAction()
    {
        try {
            ...
            if (!$wordpress->getId()) {
                throw new Nwicode_Exception('#33-002: ' . __('An error occured.'));
            }
            ...
            if (!$result || $refresh) {
                ...
                if (!$wordpressQuery->getId()) {
                    throw new Nwicode_Exception('#33-003: ' . __('An error occured.'));
                }
