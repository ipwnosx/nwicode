...
class Contact_Mobile_FormController extends Application_Controller_Mobile_Default
{
    ...
    {
        try {
            if ($datas = Nwicode_Json::decode($this->getRequest()->getRawBody())) {
                try {
                    $errors = [];
                    if (empty($datas['email']) ||
                        !Zend_Validate::is($datas['email'], 'emailAddress')) {
                        throw new Nwicode_Exception(p__("contact","Please enter a valid email address"));
                    }

                    $contact = $this->getCurrentOptionValue()->getObject();
                    if (!$contact->getId()) {
                        throw new Nwicode_Exception(p__("contact","An error occurred while sending your request. Please try again later."));
                    }
                ...
                throw new Nwicode_Exception("The sent request is empty.");
            }
        } catch (Exception $e) {
            $data = [
                "error" => true,
