...
class Loyaltycard_Mobile_ViewController extends Application_Controller_Mobile_Default
{
        ...
        try {
            ...
            if ($datas = Zend_Json::decode($this->getRequest()->getRawBody())) {
                ...
                if (empty($customer_id)) {
                    throw new Nwicode_Exception(__('You must be logged in to validate points'));
                }
                ...
                if ($card->getValueId() != $option_value->getId() OR empty($password_entered) OR !preg_match('/[0-9]/', $nbr)) {
                    throw new Nwicode_Exception(__("An error occurred while validating point<br />Please try again later."));
                }
                ...
                if (!$password->getId()) {
                    ...
                    if ($only > 0) {
                        ...
                        throw new Nwicode_Exception(__('Wrong password.<br />Be careful !<br />%d remaining attempt%s before locking your card.<br />Ask the store person for validating your point', $only, $only > 1 ? 's' : ''));
                    } else {
                        // Sinon, on ferme le clavier et on annonce que la carte est bloquée
                        $html['close_pad'] = true;
                        $html['card_is_locked'] = true;
                        throw new Nwicode_Exception(__("You have exceeded the number of attempts to validate points.<br />Your card is locked for 24h"));
                    }
                    ...
                    if ($card->getNumberOfPoints() < $card->getMaxNumberOfPoints()) {
                        ...
                        \Nwicode\Hook::trigger('loyalty_card.validate', [
                        ...
                        \Nwicode\Hook::trigger('loyalty_card.complete', [
                            'customerId' => $customer_id,
                            'card' => $card,
                            'unlockType' => 'password'
                        ]);
    ...
    protected function _getPictos($current_card)
    {
        ...
        $image_inactive = Nwicode_Image::open($path);
