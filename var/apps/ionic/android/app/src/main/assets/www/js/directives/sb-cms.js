...
angular.module('starter').directive('sbCmsText', function () {
    return {
        ...
        '   <img width="{{block.size}}%" style="width:{{block.size}}%;" ng-src="{{ block.image_url }}" ng-if="block.image.length" class="{{ block.alignment }}" />' +
    ...
    return {
        ...
        template: '<img width="100%" style="width:100%;"  ng-src="{{ block.gallery[0].src }}" alt="{{block.name}}">'
    };
