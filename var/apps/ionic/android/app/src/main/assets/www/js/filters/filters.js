 ...
 */
    ...
    function _get(emoji) {
        if (EMOJIS.hasOwnProperty(emoji)) {
            return $sce.trustAsHtml('<span class="emoji">' + EMOJIS[emoji] + '</span>');
        }
        return ':' + emoji + ':';
    }

    return function (str) {
        str = angular.isString(str) ? str : ((angular.isUndefined(str) || str === null) ? "" : Object.prototype.toString(str));
        str = str.replace(/&/g, "&amp;")
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;")
            .replace(/"/g, "&quot;")
            .replace(/'/g, "&#039;");
        return $sce.trustAsHtml(str.split(parser) // parse emoji via regex
